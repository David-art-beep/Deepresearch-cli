...
本文档列出下游消费的正式字段。额外描述字段不影响下游读取时不阻断工作流；正式消费字段仍必须类型正确。
...
- `consensus` 与 `disputes` 均可为空；只记录 Scout 实际发现的内容。
- `blanks` 可为空。
- `blank_nature`：
...
这里只记录能够通过 Research 继续核验的未知；不能靠研究解决的口径分歧应进入 `user_confirmations_needed`。
...
所有候选 lens 默认都是非绑定建议，不再重复保存固定状态字段。
...
- 四个方向数组必须显式存在，可为空。
- `scan_scope.scanned_angles` 与 `unscanned_angles` 可为空。
- `zoom_level`：`broad` / `domain` / `subdomain` / `niche`。
- `lists_known_partial` 六个子对象全部必填。

## `hypotheses_to_test`

无初步假设时使用空数组；不设机械数量上限。
...
只记录 Scout 实际发现且会影响后续规划的风险；没有实质风险时使用空数组，不得为凑满类别而生成占位项。支持的标准类型为：
