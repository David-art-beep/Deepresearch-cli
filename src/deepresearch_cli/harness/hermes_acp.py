...
class HermesAcpAttemptRuntime(AcpAttemptRuntime):
    """One Hermes ACP subprocess serving exactly one Agent attempt."""
    ...
    def __init__(
        ...
        supplied_command = hermes_command or shutil.which("hermes") or "hermes"
        ...
        self.hermes_command = resolved_command or supplied_command
    ...
    async def __aenter__(self) -> "HermesAcpAttemptRuntime":
        await self.start()
        return self
    ...
    async def preflight(self) -> Mapping[str, Any]:
        command = shutil.which(self.hermes_command) or (
            self.hermes_command if Path(self.hermes_command).is_file() else None
        )
        if not command:
            raise HarnessError(f"Hermes executable not found: {self.hermes_command}")
    ...
    def _resolved_search_provider_python(self) -> str:
        ...
        # the base uv/CPython binary would silently lose that environment's
        # site-packages.
    ...
    async def start(self) -> None:
        if self._started:
            return
        if self._closed:
            raise HarnessError("Hermes attempt runtime has already been closed")
        self.workspace.mkdir(parents=True, exist_ok=True)
        self._process_instance_id = "hermes-process-%s" % uuid.uuid4().hex
        self._stderr_chunks = []
        self._stderr_base_offset = 0
        self._stderr_end_offset = 0
        self._stderr_activity.clear()
    ...
    async def invoke(self, invocation: AgentInvocation) -> AgentExecutionResult:
        ...
        if self._invocation_claimed:
            raise HarnessError(
                "one Hermes attempt runtime cannot serve multiple invocations"
            )
        if (
            ...
            raise HarnessError(
                "Hermes attempt runtime received an unexpected invocation id"
            )
        ...
        try:
            ...
            session = await await_with_invocation_budget(
                self._connection.new_session(
                    cwd=str(invocation.workspace), mcp_servers=mcp_servers
                )
            )
            session_id = session.session_id
            ...
            self._client.reset_session(session_id)
            if search_server_name is not None:
            ...
            if invocation.allow_workspace_edits:
                set_mode = getattr(self._connection, "set_session_mode", None)
                if set_mode is None:
                    raise HarnessError(
                        "Hermes ACP does not support workspace-scoped edit mode"
                    )
                ...
                if mode_response is None:
                    raise HarnessError(
                        "Hermes rejected workspace-scoped edit mode"
                    )
            response = await await_with_invocation_budget(
                self._connection.prompt(
                    prompt=[TextContentBlock(type="text", text=invocation.prompt)],
                    session_id=session_id,
            ...
            hermes_agent_failed = session_agent_failed or provider_stream_failed
            if hermes_agent_failed:
                status = "failed"
                if provider_stream_failed:
                    error = response_text.strip() or (
                        "Hermes model-provider stream failed; "
                        "see stderr diagnostics"
                    )
                else:
                    error = (
                        "Hermes agent failed inside the ACP session; "
                ...
                status = "cancelled"
                error = "Hermes cancelled the prompt"
            else:
                status = "failed"
                error = "Hermes stopped the prompt with %s" % response.stop_reason
            ...
            return AgentExecutionResult(
                ...
                stderr=await self._settled_stderr_text(stderr_start_offset),
                error=f"Hermes invocation timed out after {invocation.timeout_seconds}s",
            )
        except asyncio.CancelledError:
            progress_status = "cancelled"
            if search_lease_file is not None:
    ...
    def _bind_process_stderr_log(self, invocation: AgentInvocation) -> str:
        ...
        if self._process_stderr_ref is not None:
            if run_workspace != self._process_stderr_workspace:
                raise HarnessError(
                    "one Hermes attempt process cannot span multiple Run workspaces"
                )
            return self._process_stderr_ref
        if self._process_instance_id is None:
            raise HarnessError("Hermes process instance id is unavailable")
